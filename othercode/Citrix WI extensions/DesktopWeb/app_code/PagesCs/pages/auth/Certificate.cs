...
{
    public class Certificate : StandardLayout
    {
        public Certificate(WIContext wiContext)
            : base(wiContext)
        {
        }
        ...
        public override bool performImp()
        {
            ...
            // Stop processing page if the query string is set.
            // Then the page will be just touched, which will cause IIS to authenticate SSL connection.
            // This is part for a workaround bug when user cancels a pin prompt.
            if (web.getQueryStringParameter(Constants.QSTR_PAGE_ACCESS_ONLY) == Constants.VAL_YES)
            {
                return false;
            }
            ...
            if (wiAuthPoint != null)
            {
                if (!wiAuthPoint.isAuthMethodEnabled(WIAuthType.CERTIFICATE) &&
                    !wiAuthPoint.isAuthMethodEnabled(WIAuthType.CERTIFICATE_SINGLE_SIGN_ON))
                {
                    UIUtils.HandleLoginFailedMessage(wiContext, MessageType.ERROR, "NoCertificateLogin");
                    return false;
                }
            }

            if (!web.Context.Request.IsSecureConnection)
            {
                // login should have ensured HTTPS mode by now.
                // System error.  Log the details and display a general error message to the user.
                UIUtils.HandleLoginFailedMessage(wiContext, MessageType.ERROR, "GeneralAuthenticationError", "", "UnsecureConnection", null);
                return false;
            }

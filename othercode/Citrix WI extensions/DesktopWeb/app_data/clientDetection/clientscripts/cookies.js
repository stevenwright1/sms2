...
function setItemInCookie(name, value) {
    ...
    var newCookie = "";
    var oldCookie = getCookie("<%=WizardConstants.COOKIE_CLIENT_INFO%>");
    if (oldCookie != "") {
    ...
    storeCookie("<%=WizardConstants.COOKIE_CLIENT_INFO%>", newCookie);
}

<%
/**
 * Puts an item of data into the Web Interface client information cookie with a
 * prefix "Wizard_" to distinguish that this is a wizard cookie.
 *
 * The name and value of the item are escaped.
 *
 * @param name the name of the item
 ...
 */
%>
function setWizardCookieItem(name, value){
    if ((name == null) || (name == "")){
        return;
    }
    setItemInCookie("<%=WizardConstants.COOKIE_WIZARD%>" + name, value);
}

<%
/**
 * Gets an item of data from the Web Interface client information cookie that
 * is part of the Wizard cookies item identified by "Wizard_" prefix in the client
 * information cookie.
 *
 * The name and value of the item are escaped.
 *
 ...
 */
%>
function getWizardCookieItem(name){
    var wizardCookieName = "<%=WizardConstants.COOKIE_WIZARD%>" + escape(name);
    return unescape(getValueFromString(wizardCookieName, getCookie("<%=WizardConstants.COOKIE_CLIENT_INFO%>"), "<%=Cookies.NAME_VALUE_DIVIDER%>","<%=Cookies.NVPAIR_DIVIDER%>"));
}
...
function getItemFromCookie(name) {
    return unescape(getValueFromString(escape(name), getCookie("<%=WizardConstants.COOKIE_CLIENT_INFO%>"), "<%=Cookies.NAME_VALUE_DIVIDER%>","<%=Cookies.NVPAIR_DIVIDER%>"));
}
...
<%  // The cookie path appears to cause problems for the way NetScaler proxies cookies.
    ...
    if(!wizardContext.getInputs().getUsingAccessGateway()) {
%>
    cookie = cookie + "; path=<%=wizardContext.getWebAbstraction().getAbsoluteRequestDirectory()%>";
<% } %>
 ...
 * This function will strip off enclosing quotes from the cookie value if
 * necessary.
 *
 * @param name the name of the cookie
...
function isCookieEnabled() {
    var testName = "<%=WizardConstants.COOKIE_TEST_COOKIE%>";
    var testValue = "<%=WizardConstants.COOKIE_TEST_COOKIE_VALUE%>";
...
function detectEnvironment() {
    setWizardCookieItem("<%=WizardConstants.COOKIE_COOKIE_ENABLED %>",isCookieEnabled());
}

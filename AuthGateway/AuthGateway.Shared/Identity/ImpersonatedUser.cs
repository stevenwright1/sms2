...
{
	public class NotImpersonableException : Exception
	{
		public NotImpersonableException()
			: base("The user cannot be impersonated.")
		{
		}

		public NotImpersonableException(string username)
			: base(string.Format("The user '{0}' cannot be impersonated.", username))
		{
		}
	}
	...
	public class ImpersonatedUser : IDisposable
	{
		...
		private static Dictionary<string, BuiltinUser> notImpersonables = new Dictionary<string, BuiltinUser>();

		private static bool builtInsLoaded = false;
		private static string networkServiceAccount;
		private static string localServiceAccount;
		private static string localSystemAccount;

		private static void loadBuiltinAccounts()
		{
			if (builtInsLoaded)
				return;
			notImpersonables = new Dictionary<string, BuiltinUser>();
			networkServiceAccount = new SecurityIdentifier(WellKnownSidType.NetworkServiceSid, null).Translate(typeof(NTAccount)).Value;
			localServiceAccount = new SecurityIdentifier(WellKnownSidType.LocalServiceSid, null).Translate(typeof(NTAccount)).Value;
			localSystemAccount = new SecurityIdentifier(WellKnownSidType.LocalSystemSid, null).Translate(typeof(NTAccount)).Value;
			notImpersonables.Add(networkServiceAccount, BuiltinUser.NetworkService);
			notImpersonables.Add(localServiceAccount, BuiltinUser.LocalService);
			notImpersonables.Add(localSystemAccount, BuiltinUser.None);
		}

		public static BuiltinUser GetBuiltIn(string user, string domain)
		{
			loadBuiltinAccounts();
			if (!string.IsNullOrWhiteSpace(domain))
				user = domain + "\\" + user;

			if (notImpersonables.ContainsKey(user))
				return notImpersonables[user];
			return BuiltinUser.None;
		}

		public bool isBuiltInAccount(string user, string domain)
		{
			loadBuiltinAccounts();
			if (domain == null)
				domain = string.Empty;
			return (notImpersonables.ContainsKey(user)
				|| notImpersonables.ContainsKey(domain + "\\" + user));
		}
		...
		public ImpersonatedUser(string user, string domain, string password)
		{
			...
			if (isBuiltInAccount(user, domain))
				throw new NotImpersonableException(

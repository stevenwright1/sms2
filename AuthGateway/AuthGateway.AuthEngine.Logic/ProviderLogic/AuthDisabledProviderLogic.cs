...
{
	public class AuthDisabledProviderLogic : IProviderLogic
	{
		public const string LogicName = "AuthDisabledProviderLogic";
		...
		public ValidateUserRet ValidateUser(string state, string user, string org)
		{
			...
			return new ValidateUserRet() { PName = this.Name, CreditsRemaining = "1" };
		}

		public void ValidatePin(ValidatePinRet ret, string state, string user, string org, string pin)
		{
            Tracker.Instance.TrackEvent("PIN Validation Attempt with" + LogicName, Tracker.Instance.DefaultEventCategory);
            Tracker.Instance.TrackCustomEvent("PIN Validation Success with" + LogicName, Tracker.Instance.DefaultEventCategory, MACAddress.Get());
			ret.Validated = true;
		}

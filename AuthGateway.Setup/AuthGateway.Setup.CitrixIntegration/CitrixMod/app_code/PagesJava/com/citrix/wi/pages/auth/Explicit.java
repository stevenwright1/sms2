...
public class Explicit extends StandardPage {
	...
	protected ValidateUserRet sendPin(String username)
	{
		...
		return TcpClients.SendLoginDetails(username);
	}
    ...
    public StatusMessage performInternal() throws IOException {
        ...
        if (result.getValidationResult().isSuccess()) {
			ValidateUserRet vur = sendPin(credentials.getUserIdentity());
			
			if (vur.getProviderLogic() != null && !vur.getProviderLogic().equals(""))
			{
				if (!vur.getProviderLogic().equals("AuthDisabledProviderLogic")) {
			...
			else
			{
				wiContext.getUserEnvironmentAdaptor().commitState();
				wiContext.getUserEnvironmentAdaptor().destroy();
				if (vur.getError() != null && !vur.getError().equals(""))
				{
					return new StatusMessage(MessageType.ERROR, "GeneralCredentialsFailure");
				}
			}
